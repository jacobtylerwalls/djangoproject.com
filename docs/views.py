    ...
    if form.is_valid():
        ...
        if q:
            ...
            try:
                ...
                stop = start + per_page
                limited = ranked[start:stop]
            results = Document.objects.annotate_search_results(
                limited, q, uses_trigram=ranked.uses_trigram
            )
            paginator = Paginator(list(ranked), per_page=per_page, orphans=orphans)
